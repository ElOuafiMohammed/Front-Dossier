...
@Component({
  selector: 'siga-montant-sub-dp',
  templateUrl: './montantSubDP.component.html',
  styleUrls: ['./montantSubDP.component.scss'],
})

export class MontantSubDPComponent implements OnInit, ViewCell {

  /**
   * Element ref du champ de saisie du montant subvention
   */
  @ViewChild('montantSubDispositif') montantSubDispositif: ElementRef;

  /**
   * index de la  colonne subvention
   */
  indexTabSubvention = 0;
  ...
  /**
  * Contrôleur du champ de saisie du montant subvention d'un DP
  */
  control: FormControl;

  /**
   * la longueur du champ de saisie du montant subvention d'un DP
   */
  valueLength: any;
  ...
  */
  ...
  /**
   * Initialisation du composant montantSubvention
   */
  ngOnInit() {
  ...
  /**
 * Fonction qui détecte les changements de valeur dans le contrôleur montant Subvention
 */
  onBlurEvent() {
    ...
    if (this.control.value) {
      this.rowData.montantSubvention = NumberUtils.toNumber(value);
  ...
  * @param control: Contrôleur du champ courant "montantSubvention"
  */
    ...
    if (this.control.value === 0) {
      ...
      if (event && event.target) {
        if (event.target.selectionEnd === event.target.selectionStart) {
          ...
          event.target.selectionEnd = positionInit;
          this.montantSubDispositif.nativeElement.setSelectionRange(positionInit, positionInit, 'none');
        } else {
